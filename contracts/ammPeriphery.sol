...
contract Router {
    ...
    function getTokenPrice(
        ...
        returns (
            uint reserveA,
            uint reserveB,
            uint one_tokenA_price,
            uint one_tokenB_price
        )
    {
    ...
    function calculateLpTokenAddAmount(
        ...
        address _tokenB,
        uint _amountA
    ) public view returns (uint _amountB) {
        address lptokenAddr = amm.getLptoken(_tokenA, _tokenB);
        _amountB =
            (amm.getReserve(lptokenAddr, _tokenB) * _amountA) /
            amm.getReserve(lptokenAddr, _tokenA);
    }
}
